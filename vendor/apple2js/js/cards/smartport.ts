...
function asMemoryDisk(disk: BlockDisk | undefined | null): MemoryBlockDisk | null {
    // Duck-type: Vite can duplicate the MemoryBlockDisk class across chunks,
    // so `instanceof` is not reliable in this app.
    if (disk && typeof (disk as MemoryBlockDisk).readSync === 'function') {
        return disk as MemoryBlockDisk;
    }
    return null;
}
    ...
    implements Card, BlockStorage, Restorable<SmartPortState>
{
    ...
    getDeviceInfoSync(driveNo: DriveNumber): number {
    ...
    formatDeviceSync(driveNo: DriveNumber): number {
        const mem = asMemoryDisk(this.disks[driveNo]);
        if (!mem) {
            debug('Drive', driveNo, 'is empty');
            return DEVICE_OFFLINE;
        ...
        for (let idx = 0; idx < blockCount; idx++) {
            mem.writeSync(idx, new Uint8Array(512));
        }

        return 0;
    }

    handleAsync(fn: () => Promise<number> | number): void {
        this.xReg = 0x00;
        this.yReg = 0x00;
        try {
            const result = fn();
    ...
    read(_page: byte, off: byte) {
        ...
        if (off === blockOff && this.cpu.getSync()) {
            ...
            switch (cmd) {
                ...
                case 1: // READ
                    this.handleAsync(() =>
                        this.readBlockSync(drive, block, buffer)
                    );
                    break;

                case 2: // WRITE
                    this.handleAsync(() =>
                        this.writeBlockSync(drive, block, buffer)
                    );
                    break;

                case 3: // FORMAT
                    this.handleAsync(() => this.formatDeviceSync(drive));
                    break;
            }
