...
//#region node_modules/.nitro/vite/services/ssr/assets/thunderclock-iUjUCgFh.js
...
var Thunderclock = class {
	...
	register = 0;
	bits = [];
	command = COMMANDS.REGHOLD;
	debug(..._args) {}
	calcBits() {
		const shift = (val) => {
			for (let idx = 0; idx < 4; idx++) {
				this.bits.push((val & 8) !== 0);
				val <<= 1;
			}
		};
		const shiftBCD = (val) => {
						...
						if (strobe) {
							...
							switch (this.command) {
								...
								case COMMANDS.REGHOLD:
									...
									break;
								default: this.debug("Unknown command", toHex(this.command));
							}
						}
					}
	...
	read(page, off) {
		let result;
		if (page < 200) result = rom[off];
		else result = rom[page - 200 << 8 | off];
		return result;
	}
	write() {}
	ioSwitch(off, val) {
...
//#endregion
export { Thunderclock as default };
